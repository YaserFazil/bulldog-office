...
def main():
    ...
    if selected_username:
        ...
        work_history = fetch_employee_work_history(user_id)
        ...
        if period_loaded:
            ...
            work_history = fetch_employee_work_history(user_id, pay_period_from_selected, pay_period_to_selected)

            st.session_state["edited_work_history_data"] = work_history
            st.rerun()

        if "edited_work_history_data" not in st.session_state:
            st.session_state["edited_work_history_data"] = work_history
        ...
        with col3:
            ...
            if st.button("Save Changes", use_container_width=True):
                ...
                if work_history_created["success"] == True:
                    ...
                    st.session_state["edited_work_history_data"] = fetch_employee_work_history(user_id)
                    st.rerun()
                else:
